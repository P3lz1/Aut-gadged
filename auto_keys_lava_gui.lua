...
local actions = {
    { name = "E", key = Enum.KeyCode.E, vk = 0x45, enabled = false, speed = 5 },
    { name = "R", key = Enum.KeyCode.R, vk = 0x52, enabled = false, speed = 5 },
    { name = "Q", key = Enum.KeyCode.Q, vk = 0x51, enabled = false, speed = 5 },
    { name = "C", key = Enum.KeyCode.C, vk = 0x43, enabled = false, speed = 5 },
    { name = "V", key = Enum.KeyCode.V, vk = 0x56, enabled = false, speed = 5 },
...
local function pressKey(action)
    if type(keypress) == "function" and type(keyrelease) == "function" and action.vk then
        pcall(function()
            keypress(action.vk)
        end)
...
local function setLeftClickHold(state)
    ...
    if state and type(mouse1press) == "function" then
        pcall(mouse1press)
    elseif not state and type(mouse1release) == "function" then
        pcall(mouse1release)
    else
...
local function releaseAll()
    for _, action in ipairs(actions) do
        if action.key then
            pcall(function()
                if type(keyrelease) == "function" and action.vk then
                    keyrelease(action.vk)
                else
                    VirtualInputManager:SendKeyEvent(false, action.key, false, game)
                end
