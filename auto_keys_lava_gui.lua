...
local lavaClips = {
    { rect = Vector2.new(0, 47), size = Vector2.new(165, 13), y = 64, w = 172, h = 16, speed = 3.3, delay = 0.0 },
    { rect = Vector2.new(92, 63), size = Vector2.new(175, 16), y = 94, w = 188, h = 18, speed = 2.9, delay = 0.8 },
    { rect = Vector2.new(12, 83), size = Vector2.new(210, 14), y = 132, w = 220, h = 16, speed = 3.6, delay = 0.35 },
    { rect = Vector2.new(124, 103), size = Vector2.new(130, 12), y = 179, w = 142, h = 14, speed = 2.8, delay = 1.1 },
    { rect = Vector2.new(28, 128), size = Vector2.new(180, 10), y = 228, w = 192, h = 12, speed = 3.1, delay = 0.55 },
    { rect = Vector2.new(115, 141), size = Vector2.new(145, 9), y = 269, w = 156, h = 11, speed = 2.5, delay = 1.45 },
}

for _, clip in ipairs(lavaClips) do
    local strip = Instance.new("ImageLabel")
    strip.Size = UDim2.new(0, clip.w, 0, clip.h)
    strip.Position = UDim2.new(0, 292, 0, clip.y)
    strip.BackgroundTransparency = 1
    strip.Image = BACKGROUND_IMAGE
    strip.ImageRectOffset = clip.rect
    strip.ImageRectSize = clip.size
    strip.ImageColor3 = Color3.fromRGB(255, 115, 65)
    strip.ImageTransparency = 0.18
    strip.ScaleType = Enum.ScaleType.Stretch
    strip.ZIndex = 3
    strip.Parent = frame

    task.spawn(function()
        task.wait(clip.delay)
        ...
        while strip.Parent do
            ...
            local tween = TweenService:Create(
                strip,
                TweenInfo.new(clip.speed, Enum.EasingStyle.Linear, Enum.EasingDirection.Out),
                { Position = UDim2.new(0, -clip.w, 0, clip.y) }
            )
